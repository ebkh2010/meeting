...
# بالا آوردن سرویس‌ها ← ساخت باکت‌های ذخیره‌سازی ← بررسی سلامت ← فعال‌سازی گواهی SSL.
#
...
# سامانه روی پورت میزبان APP_PORT منتشر شده است (پیش‌فرض ۷۰۸۰).
if curl -fsS -H "Host: ${APP_DOMAIN}" "http://127.0.0.1:${APP_PORT:-7080}/health" >/dev/null 2>&1; then
    ok "پروکسی درست به بک‌اند وصل است."
else
    warn "پاسخ سلامت از پروکسی دریافت نشد؛ پورت ${APP_PORT:-7080} و فایروال را بررسی کنید."
fi

# --------------------------------------------------------------------------
# ۶) گواهی SSL (دستی — سرویس certbot در این بسته وجود ندارد)
# --------------------------------------------------------------------------
if [ "${SKIP_SSL}" = "1" ]; then
    warn "فعال‌سازی گواهی SSL به‌درخواست شما رد شد (SKIP_SSL=1). سامانه فقط با HTTP کار می‌کند."
else
    log "فعال‌سازی گواهی SSL برای ${APP_DOMAIN} و ${STORAGE_DOMAIN}..."
    if bash "${SCRIPT_DIR}/issue-ssl.sh"; then
        ok "گواهی فعال و HTTPS برقرار شد."
    else
        warn "گواهی دستی در deploy/nginx/certs قرار نگرفته است."
        warn "پس از قرار دادن fullchain.pem و privkey.pem اجرا کنید: bash scripts/issue-ssl.sh"
        warn "یا TLS را در لبه (nginx میزبان/CDN) خاتمه دهید و لبه را به پورت‌های ${APP_PORT:-7080} و ${STORAGE_HOST_PORT:-7443} پراکسی کنید."
    fi
fi
