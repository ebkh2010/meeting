...
- **نسخه:** ۱.۰ | **تاریخ:** 2026-08-17 | **تهیه‌کننده:** ابراهیم خرم نژاد (معمار نرم‌افزار)
- **ورودی مبنا:** `docs/mvp_feature_review.md` (بخش‌های ۵.۱، ۶.۱، ۶.۳)، `.notes/ATOMS.md`، `.notes/PROGRESS.md`
...
> این سند مرجع طراحی است. **وضعیت پیاده‌سازی واقعی (تا v1.0.26)** در بخش زیر
> آمده است؛ هر جا این بخش با بدنهٔ سند اختلاف دارد، وضعیت پیاده‌سازی ملاک است.

---

## ۰. وضعیت پیاده‌سازی (تا v1.0.26 — آخرین به‌روزرسانی ۱۴۰۵/۰۶/۱۶)

سیستم روی سرور تولید (212.80.18.20) مستقر و با هارنس E2E (۲۱۰ PASS / ۶ FAIL
شناخته‌شده) آزموده شده است. مهم‌ترین تفاوت‌های واقعیت پیاده‌سازی با این سند طراحی:

| موضوع | در سند طراحی | واقعیت پیاده‌سازی |
|---|---|---|
| صف کار | Celery 5 + Redis 7 + Worker جدا | جدول پایدار `jobs` + اجرای in-process با `asyncio.create_task` در همان پروسهٔ FastAPI؛ وضعیت/پیشرفت/retry در DB؛ بازیابی کارهای نیمه‌کاره هنگام بالا آمدن (`RECOVER_ORPHAN_JOBS`). بدون Redis/Celery |
| ORM | SQLModel | SQLAlchemy 2 (Core + ORM) مستقیم |
| مهاجرت | Alembic | `Base.metadata.create_all` برای جداول جدید + مهاجرت‌های `ALTER TABLE` دستی هنگام استقرار |
| جداسازی مستأجر | Repository + **RLS** | فقط لایهٔ کد: `resolve_context` / `list_owned` / `get_owned` با شرط `organization_id` (RLS فعال نشده است) |
| Proxy/TLS | Caddy 2 + TLS خودکار | nginx در کانتینر؛ گواهی دستی در `deploy/nginx/certs` یا خاتمهٔ TLS در لبه (nginx میزبان/CDN) |
| PDF فارسی | WeasyPrint | خروجی **Word واقعی RTL** با `python-docx` (`services/minutes_docx.py`) + «نمای چاپ/PDF» مرورگر؛ فایل ICS با `icalendar` |
| کش توکن «حرف» | Redis | توکن در حافظهٔ فرایند کش می‌شود؛ در ۴۰۱ یک‌بار ورود مجدد انجام می‌شود |
| نرمال‌سازی فارسی | تابع SQL `fa_normalize` + FTS GIN | تابع پایتونی `fa_normalize` در `services/mgmt_core.py` (۱:۱) + نسخهٔ فرانت در `lib/utils.ts`؛ جست‌وجو در پایتون روی متن‌های جمع‌شده |
| احراز نشست | JWT کوتاه‌عمر + Refresh چرخشی | توکن JWT (`typ=vidara_app` برای فضای کاری، `typ=vidara_platform` برای مدیر پلتفرم) با TTL طولانی‌تر؛ ورود با جفت «نام کاربری + رمز» بدون مرحلهٔ انتخاب سازمان |
| نقش‌ها | سه نقش مستأجر | سه نقش سازمان (`org_admin`/`secretary`/`member`) + نقش مستقل **مدیر پلتفرم** (جدول `platform_admins`، ورود از همان صفحه، صفحهٔ `/platform`) |
| دیاریزیشن | خارج از دامنهٔ MVP | پیاده شده: برچسب گوینده از «حرف» + نام‌گذاری گوینده توسط مدیر جلسه + کلیپ نمونهٔ صدا (ffmpeg) |

**قابلیت‌هایی که فراتر از این سند پیاده شده‌اند:**

- **واحد یکپارچهٔ «توکن ویدارا»:** ۱ دقیقه رونویسی = ۱ توکن، ۱ سنت مدل زبانی = ۱ توکن؛ سقف ماهانهٔ هر کاربر + سقف سازمان؛ پنل مصرف در داشبورد کاربر و «مصرف هوش مصنوعی» پنل پلتفرم (مجموع حرف/دیپ‌سیک + معادل دلاری نرخ روز از صفحهٔ رسمی قیمت‌گذاری DeepSeek).
- **پنل مدیریت پلتفرم** (`/api/v1/platform`): ساخت مدیر سازمان (نام/نام خانوادگی/موبایل → یوزر = موبایل، رمز رندوم پیامک‌شده، تکمیل اجباری مشخصات در نخستین ورود)، فهرست سازمان‌ها و مدیران، ویرایش تنظیمات ایمیل/پیامک/AI/استوریج و سقف‌های هر سازمان، مصرف AI کل پلتفرم، و «سطل آشغال» با بازیابی و پاک‌سازی کامل (داده‌ها + فایل‌های Storage).
- **جست‌وجوی تمام‌متن جلسات** با فیلتر محدوده (نام/دستور/صورتجلسه/رونویسی/مصوبات/اقدامات)، نمایش بندهای یافت‌شده با برچسب منبع، و **پرش به همان بخش** در صفحهٔ جلسه با هایلایت واژه.
- **چند فایل صوتی در یک جلسه با ترتیب دلخواه کاربر:** رونویسی مستقل و همزمان هر فایل؛ متن نهایی = ترکیب فایل‌های رونویسی‌شده به همان ترتیب؛ صورت‌جلسه از متن نهایی تجمیعی ساخته می‌شود.
- **آپلود ویدیوی جلسه:** جداسازی خودکار صدا با ffmpeg (کار `audio_extract`)، رونویسی از صدای استخراج‌شده، و «حذف ویدیو با نگه‌داری صدا».
- **پیام خطای صریح برای هر شکست آپلود** (فرمت/حجم/مدت/شبکه/استوریج) در لحظهٔ انتخاب و زیر هر فایل ناموفق.
- **تنظیمات تولید صورت‌جلسه به‌ازای هر جلسه** (`meeting_minutes_settings`) و پیشنهاد هوشمند مصوبات/اقدامات با بازبینی پیش از ثبت.
- **دستیار هوشمند** با بازیابی محتوای واقعی سازمان (RAG سبک بدون سرویس برداری) و انتخاب متناسب قطعه‌ها بین جلسات.
