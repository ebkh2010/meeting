...
# ------------------ Service Layer ------------------
class Org_notify_settingsService:
    """Service layer for Org_notify_settings operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Org_notify_settings]:
        """Create a new org_notify_settings"""
        try:
            obj = Org_notify_settings(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created org_notify_settings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating org_notify_settings: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Org_notify_settings]:
        """Get org_notify_settings by ID"""
        try:
            query = select(Org_notify_settings).where(Org_notify_settings.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_notify_settings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of org_notify_settingss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Org_notify_settings, field):
                        column = getattr(Org_notify_settings, field)
            ...
            stmt = select(Org_notify_settings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Org_notify_settings, field_name):
                        stmt = stmt.order_by(getattr(Org_notify_settings, field_name).desc())
                else:
                    if hasattr(Org_notify_settings, sort):
                        stmt = stmt.order_by(getattr(Org_notify_settings, sort))
            else:
                stmt = stmt.order_by(Org_notify_settings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Org_notify_settings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching org_notify_settings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Org_notify_settings]:
        """Update org_notify_settings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_notify_settings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated org_notify_settings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating org_notify_settings {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete org_notify_settings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_notify_settings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted org_notify_settings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting org_notify_settings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Org_notify_settings]:
        """Get org_notify_settings by any field"""
        try:
            if not hasattr(Org_notify_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_notify_settings")
            column = getattr(Org_notify_settings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_notify_settings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_notify_settings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Org_notify_settings]:
        """Get list of org_notify_settingss filtered by field"""
        try:
            if not hasattr(Org_notify_settings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_notify_settings")
            column = getattr(Org_notify_settings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_notify_settings)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Org_notify_settings.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching org_notify_settingss by {field_name}: {str(e)}")
            raise