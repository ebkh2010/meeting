...
# ------------------ Service Layer ------------------
class Notify_deliveriesService:
    """Service layer for Notify_deliveries operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Notify_deliveries]:
        """Create a new notify_deliveries"""
        try:
            obj = Notify_deliveries(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created notify_deliveries with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating notify_deliveries: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Notify_deliveries]:
        """Get notify_deliveries by ID"""
        try:
            query = select(Notify_deliveries).where(Notify_deliveries.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching notify_deliveries {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of notify_deliveriess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Notify_deliveries, field):
                        column = getattr(Notify_deliveries, field)
            ...
            stmt = select(Notify_deliveries, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Notify_deliveries, field_name):
                        stmt = stmt.order_by(getattr(Notify_deliveries, field_name).desc())
                else:
                    if hasattr(Notify_deliveries, sort):
                        stmt = stmt.order_by(getattr(Notify_deliveries, sort))
            else:
                stmt = stmt.order_by(Notify_deliveries.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Notify_deliveries)
                for condition in conditions:
            ...
            logger.error(f"Error fetching notify_deliveries list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Notify_deliveries]:
        """Update notify_deliveries"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Notify_deliveries {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated notify_deliveries {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating notify_deliveries {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete notify_deliveries"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Notify_deliveries {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted notify_deliveries {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting notify_deliveries {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Notify_deliveries]:
        """Get notify_deliveries by any field"""
        try:
            if not hasattr(Notify_deliveries, field_name):
                raise ValueError(f"Field {field_name} does not exist on Notify_deliveries")
            column = getattr(Notify_deliveries, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Notify_deliveries).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching notify_deliveries by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Notify_deliveries]:
        """Get list of notify_deliveriess filtered by field"""
        try:
            if not hasattr(Notify_deliveries, field_name):
                raise ValueError(f"Field {field_name} does not exist on Notify_deliveries")
            column = getattr(Notify_deliveries, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Notify_deliveries)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Notify_deliveries.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching notify_deliveriess by {field_name}: {str(e)}")
            raise