...
# ------------------ Service Layer ------------------
class Agenda_itemsService:
    """Service layer for Agenda_items operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Agenda_items]:
        """Create a new agenda_items"""
        try:
            obj = Agenda_items(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created agenda_items with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating agenda_items: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Agenda_items]:
        """Get agenda_items by ID"""
        try:
            query = select(Agenda_items).where(Agenda_items.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching agenda_items {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of agenda_itemss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Agenda_items, field):
                        column = getattr(Agenda_items, field)
            ...
            stmt = select(Agenda_items, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Agenda_items, field_name):
                        stmt = stmt.order_by(getattr(Agenda_items, field_name).desc())
                else:
                    if hasattr(Agenda_items, sort):
                        stmt = stmt.order_by(getattr(Agenda_items, sort))
            else:
                stmt = stmt.order_by(Agenda_items.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Agenda_items)
                for condition in conditions:
            ...
            logger.error(f"Error fetching agenda_items list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Agenda_items]:
        """Update agenda_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Agenda_items {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated agenda_items {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating agenda_items {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Agenda_items {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted agenda_items {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting agenda_items {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Agenda_items]:
        """Get agenda_items by any field"""
        try:
            if not hasattr(Agenda_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Agenda_items")
            column = getattr(Agenda_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Agenda_items).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching agenda_items by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Agenda_items]:
        """Get list of agenda_itemss filtered by field"""
        try:
            if not hasattr(Agenda_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Agenda_items")
            column = getattr(Agenda_items, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Agenda_items.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching agenda_itemss by {field_name}: {str(e)}")
            raise