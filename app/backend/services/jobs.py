...
class JobsService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Jobs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Jobs, field_name):
                        stmt = stmt.order_by(getattr(Jobs, field_name).desc())
                else:
                    if hasattr(Jobs, sort):
                        stmt = stmt.order_by(getattr(Jobs, sort))
            else:
                stmt = stmt.order_by(Jobs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Jobs)
                for condition in conditions:
