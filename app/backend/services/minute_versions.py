...
# ------------------ Service Layer ------------------
class Minute_versionsService:
    """Service layer for Minute_versions operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Minute_versions]:
        """Create a new minute_versions"""
        try:
            obj = Minute_versions(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created minute_versions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating minute_versions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Minute_versions]:
        """Get minute_versions by ID"""
        try:
            query = select(Minute_versions).where(Minute_versions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching minute_versions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of minute_versionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Minute_versions, field):
                        column = getattr(Minute_versions, field)
            ...
            stmt = select(Minute_versions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Minute_versions, field_name):
                        stmt = stmt.order_by(getattr(Minute_versions, field_name).desc())
                else:
                    if hasattr(Minute_versions, sort):
                        stmt = stmt.order_by(getattr(Minute_versions, sort))
            else:
                stmt = stmt.order_by(Minute_versions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Minute_versions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching minute_versions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Minute_versions]:
        """Update minute_versions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Minute_versions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated minute_versions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating minute_versions {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete minute_versions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Minute_versions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted minute_versions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting minute_versions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Minute_versions]:
        """Get minute_versions by any field"""
        try:
            if not hasattr(Minute_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Minute_versions")
            column = getattr(Minute_versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Minute_versions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching minute_versions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Minute_versions]:
        """Get list of minute_versionss filtered by field"""
        try:
            if not hasattr(Minute_versions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Minute_versions")
            column = getattr(Minute_versions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Minute_versions)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Minute_versions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching minute_versionss by {field_name}: {str(e)}")
            raise