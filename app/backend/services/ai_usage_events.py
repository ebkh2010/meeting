...
# ------------------ Service Layer ------------------
class Ai_usage_eventsService:
    """Service layer for Ai_usage_events operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Ai_usage_events]:
        """Create a new ai_usage_events"""
        try:
            obj = Ai_usage_events(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created ai_usage_events with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating ai_usage_events: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Ai_usage_events]:
        """Get ai_usage_events by ID"""
        try:
            query = select(Ai_usage_events).where(Ai_usage_events.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ai_usage_events {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of ai_usage_eventss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Ai_usage_events, field):
                        column = getattr(Ai_usage_events, field)
            ...
            stmt = select(Ai_usage_events, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Ai_usage_events, field_name):
                        stmt = stmt.order_by(getattr(Ai_usage_events, field_name).desc())
                else:
                    if hasattr(Ai_usage_events, sort):
                        stmt = stmt.order_by(getattr(Ai_usage_events, sort))
            else:
                stmt = stmt.order_by(Ai_usage_events.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Ai_usage_events)
                for condition in conditions:
            ...
            logger.error(f"Error fetching ai_usage_events list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Ai_usage_events]:
        """Update ai_usage_events"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Ai_usage_events {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated ai_usage_events {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating ai_usage_events {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Ai_usage_events {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted ai_usage_events {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting ai_usage_events {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Ai_usage_events]:
        """Get ai_usage_events by any field"""
        try:
            if not hasattr(Ai_usage_events, field_name):
                raise ValueError(f"Field {field_name} does not exist on Ai_usage_events")
            column = getattr(Ai_usage_events, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Ai_usage_events).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ai_usage_events by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Ai_usage_events]:
        """Get list of ai_usage_eventss filtered by field"""
        try:
            if not hasattr(Ai_usage_events, field_name):
                raise ValueError(f"Field {field_name} does not exist on Ai_usage_events")
            column = getattr(Ai_usage_events, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Ai_usage_events)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Ai_usage_events.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching ai_usage_eventss by {field_name}: {str(e)}")
            raise