...
class MeetingsService:
    """Service layer for Meetings operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Meetings]:
        ...
        try:
            ...
            logger.info(f"Created meetings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating meetings: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Meetings]:
        """Get meetings by ID"""
        try:
            query = select(Meetings).where(Meetings.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meetings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of meetingss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Meetings, field):
                        column = getattr(Meetings, field)
            ...
            stmt = select(Meetings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Meetings, field_name):
                        stmt = stmt.order_by(getattr(Meetings, field_name).desc())
                else:
                    if hasattr(Meetings, sort):
                        stmt = stmt.order_by(getattr(Meetings, sort))
            else:
                stmt = stmt.order_by(Meetings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Meetings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching meetings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Meetings]:
        """Update meetings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meetings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated meetings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating meetings {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meetings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted meetings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting meetings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Meetings]:
        """Get meetings by any field"""
        try:
            if not hasattr(Meetings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meetings")
            column = getattr(Meetings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Meetings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meetings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Meetings]:
        """Get list of meetingss filtered by field"""
        try:
            if not hasattr(Meetings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meetings")
            column = getattr(Meetings, field_name)
            ...
            logger.error(f"Error fetching meetingss by {field_name}: {str(e)}")
            raise