...
# ------------------ Service Layer ------------------
class Action_itemsService:
    """Service layer for Action_items operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Action_items]:
        """Create a new action_items"""
        try:
            obj = Action_items(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created action_items with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating action_items: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Action_items]:
        """Get action_items by ID"""
        try:
            query = select(Action_items).where(Action_items.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching action_items {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of action_itemss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Action_items, field):
                        column = getattr(Action_items, field)
            ...
            stmt = select(Action_items, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Action_items, field_name):
                        stmt = stmt.order_by(getattr(Action_items, field_name).desc())
                else:
                    if hasattr(Action_items, sort):
                        stmt = stmt.order_by(getattr(Action_items, sort))
            else:
                stmt = stmt.order_by(Action_items.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Action_items)
                for condition in conditions:
            ...
            logger.error(f"Error fetching action_items list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Action_items]:
        """Update action_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Action_items {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated action_items {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating action_items {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Action_items {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted action_items {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting action_items {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Action_items]:
        """Get action_items by any field"""
        try:
            if not hasattr(Action_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Action_items")
            column = getattr(Action_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Action_items).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching action_items by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Action_items]:
        """Get list of action_itemss filtered by field"""
        try:
            if not hasattr(Action_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Action_items")
            column = getattr(Action_items, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Action_items.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching action_itemss by {field_name}: {str(e)}")
            raise