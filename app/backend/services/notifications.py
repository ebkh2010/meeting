...
class NotificationsService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Notifications, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Notifications, field_name):
                        stmt = stmt.order_by(getattr(Notifications, field_name).desc())
                else:
                    if hasattr(Notifications, sort):
                        stmt = stmt.order_by(getattr(Notifications, sort))
            else:
                stmt = stmt.order_by(Notifications.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Notifications)
                for condition in conditions:
