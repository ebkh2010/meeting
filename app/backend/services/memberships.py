...
class MembershipsService:
    """Service layer for Memberships operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Memberships]:
        """Create a new memberships"""
        try:
            obj = Memberships(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created memberships with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating memberships: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Memberships]:
        """Get memberships by ID"""
        try:
            query = select(Memberships).where(Memberships.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching memberships {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of membershipss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Memberships, field):
                        column = getattr(Memberships, field)
            ...
            stmt = select(Memberships, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Memberships, field_name):
                        stmt = stmt.order_by(getattr(Memberships, field_name).desc())
                else:
                    if hasattr(Memberships, sort):
                        stmt = stmt.order_by(getattr(Memberships, sort))
            else:
                stmt = stmt.order_by(Memberships.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Memberships)
                for condition in conditions:
            ...
            logger.error(f"Error fetching memberships list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Memberships]:
        """Update memberships"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Memberships {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated memberships {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating memberships {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Memberships {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted memberships {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting memberships {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Memberships]:
        """Get memberships by any field"""
        try:
            if not hasattr(Memberships, field_name):
                raise ValueError(f"Field {field_name} does not exist on Memberships")
            column = getattr(Memberships, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Memberships).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching memberships by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Memberships]:
        """Get list of membershipss filtered by field"""
        try:
            if not hasattr(Memberships, field_name):
                raise ValueError(f"Field {field_name} does not exist on Memberships")
            column = getattr(Memberships, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Memberships.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching membershipss by {field_name}: {str(e)}")
            raise