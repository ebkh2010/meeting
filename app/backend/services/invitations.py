...
class InvitationsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Invitations]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Invitations]:
    ...
    async def get_list(
        ...
        """Get paginated list of invitationss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Invitations, field):
                        column = getattr(Invitations, field)
            ...
            stmt = select(Invitations, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Invitations, field_name):
                        stmt = stmt.order_by(getattr(Invitations, field_name).desc())
                else:
                    if hasattr(Invitations, sort):
                        stmt = stmt.order_by(getattr(Invitations, sort))
            else:
                stmt = stmt.order_by(Invitations.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Invitations)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Invitations]:
        """Update invitations"""
        try:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Invitations]:
        ...
        try:
            ...
            column = getattr(Invitations, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Invitations).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
