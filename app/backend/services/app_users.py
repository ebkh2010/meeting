...
class App_usersService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(App_users, field):
                        column = getattr(App_users, field)
            ...
            stmt = select(App_users, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(App_users, field_name):
                        stmt = stmt.order_by(getattr(App_users, field_name).desc())
                else:
                    if hasattr(App_users, sort):
                        stmt = stmt.order_by(getattr(App_users, sort))
            else:
                stmt = stmt.order_by(App_users.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(App_users)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[App_users]:
        ...
        try:
            ...
            column = getattr(App_users, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(App_users).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
