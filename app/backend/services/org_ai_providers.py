...
# ------------------ Service Layer ------------------
class Org_ai_providersService:
    """Service layer for Org_ai_providers operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Org_ai_providers]:
        """Create a new org_ai_providers"""
        try:
            obj = Org_ai_providers(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created org_ai_providers with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating org_ai_providers: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Org_ai_providers]:
        """Get org_ai_providers by ID"""
        try:
            query = select(Org_ai_providers).where(Org_ai_providers.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_ai_providers {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of org_ai_providerss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Org_ai_providers, field):
                        column = getattr(Org_ai_providers, field)
            ...
            stmt = select(Org_ai_providers, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Org_ai_providers, field_name):
                        stmt = stmt.order_by(getattr(Org_ai_providers, field_name).desc())
                else:
                    if hasattr(Org_ai_providers, sort):
                        stmt = stmt.order_by(getattr(Org_ai_providers, sort))
            else:
                stmt = stmt.order_by(Org_ai_providers.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Org_ai_providers)
                for condition in conditions:
            ...
            logger.error(f"Error fetching org_ai_providers list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Org_ai_providers]:
        """Update org_ai_providers"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_ai_providers {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated org_ai_providers {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating org_ai_providers {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete org_ai_providers"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_ai_providers {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted org_ai_providers {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting org_ai_providers {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Org_ai_providers]:
        """Get org_ai_providers by any field"""
        try:
            if not hasattr(Org_ai_providers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_ai_providers")
            column = getattr(Org_ai_providers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_ai_providers).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_ai_providers by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Org_ai_providers]:
        """Get list of org_ai_providerss filtered by field"""
        try:
            if not hasattr(Org_ai_providers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_ai_providers")
            column = getattr(Org_ai_providers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_ai_providers)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Org_ai_providers.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching org_ai_providerss by {field_name}: {str(e)}")
            raise