...
class ParticipantsService:
    """Service layer for Participants operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Participants]:
        """Create a new participants"""
        try:
            obj = Participants(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created participants with id: {obj.id}")
            return obj
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Participants]:
        """Get participants by ID"""
        try:
            query = select(Participants).where(Participants.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching participants {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of participantss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Participants, field):
                        column = getattr(Participants, field)
            ...
            stmt = select(Participants, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Participants, field_name):
                        stmt = stmt.order_by(getattr(Participants, field_name).desc())
                else:
                    if hasattr(Participants, sort):
                        stmt = stmt.order_by(getattr(Participants, sort))
            else:
                stmt = stmt.order_by(Participants.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Participants)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Participants]:
        """Update participants"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Participants {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated participants {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating participants {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Participants {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted participants {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting participants {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Participants]:
        """Get participants by any field"""
        try:
            if not hasattr(Participants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Participants")
            column = getattr(Participants, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Participants).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching participants by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Participants]:
        """Get list of participantss filtered by field"""
        try:
            if not hasattr(Participants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Participants")
            column = getattr(Participants, field_name)
            ...
            logger.error(f"Error fetching participantss by {field_name}: {str(e)}")
            raise