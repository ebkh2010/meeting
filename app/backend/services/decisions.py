...
class DecisionsService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Decisions]:
        ...
        try:
            ...
            logger.info(f"Created decisions with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating decisions: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Decisions]:
        """Get decisions by ID"""
        try:
            query = select(Decisions).where(Decisions.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching decisions {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of decisionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Decisions, field):
                        column = getattr(Decisions, field)
            ...
            stmt = select(Decisions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Decisions, field_name):
                        stmt = stmt.order_by(getattr(Decisions, field_name).desc())
                else:
                    if hasattr(Decisions, sort):
                        stmt = stmt.order_by(getattr(Decisions, sort))
            else:
                stmt = stmt.order_by(Decisions.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Decisions)
                for condition in conditions:
            ...
            logger.error(f"Error fetching decisions list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Decisions]:
        """Update decisions"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Decisions {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated decisions {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating decisions {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Decisions {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted decisions {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting decisions {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Decisions]:
        """Get decisions by any field"""
        try:
            if not hasattr(Decisions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Decisions")
            column = getattr(Decisions, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Decisions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching decisions by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Decisions]:
        """Get list of decisionss filtered by field"""
        try:
            if not hasattr(Decisions, field_name):
                raise ValueError(f"Field {field_name} does not exist on Decisions")
            column = getattr(Decisions, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Decisions.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching decisionss by {field_name}: {str(e)}")
            raise