...
# ------------------ Service Layer ------------------
class Org_storage_targetsService:
    """Service layer for Org_storage_targets operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Org_storage_targets]:
        """Create a new org_storage_targets"""
        try:
            obj = Org_storage_targets(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created org_storage_targets with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating org_storage_targets: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Org_storage_targets]:
        """Get org_storage_targets by ID"""
        try:
            query = select(Org_storage_targets).where(Org_storage_targets.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_storage_targets {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of org_storage_targetss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Org_storage_targets, field):
                        column = getattr(Org_storage_targets, field)
            ...
            stmt = select(Org_storage_targets, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Org_storage_targets, field_name):
                        stmt = stmt.order_by(getattr(Org_storage_targets, field_name).desc())
                else:
                    if hasattr(Org_storage_targets, sort):
                        stmt = stmt.order_by(getattr(Org_storage_targets, sort))
            else:
                stmt = stmt.order_by(Org_storage_targets.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Org_storage_targets)
                for condition in conditions:
            ...
            logger.error(f"Error fetching org_storage_targets list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Org_storage_targets]:
        """Update org_storage_targets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_storage_targets {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated org_storage_targets {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating org_storage_targets {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete org_storage_targets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_storage_targets {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted org_storage_targets {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting org_storage_targets {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Org_storage_targets]:
        """Get org_storage_targets by any field"""
        try:
            if not hasattr(Org_storage_targets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_storage_targets")
            column = getattr(Org_storage_targets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_storage_targets).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_storage_targets by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Org_storage_targets]:
        """Get list of org_storage_targetss filtered by field"""
        try:
            if not hasattr(Org_storage_targets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_storage_targets")
            column = getattr(Org_storage_targets, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Org_storage_targets.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching org_storage_targetss by {field_name}: {str(e)}")
            raise