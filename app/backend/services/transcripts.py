...
class TranscriptsService:
    """Service layer for Transcripts operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Transcripts]:
        """Create a new transcripts"""
        try:
            obj = Transcripts(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created transcripts with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating transcripts: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Transcripts]:
        """Get transcripts by ID"""
        try:
            query = select(Transcripts).where(Transcripts.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching transcripts {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of transcriptss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Transcripts, field):
                        column = getattr(Transcripts, field)
            ...
            stmt = select(Transcripts, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Transcripts, field_name):
                        stmt = stmt.order_by(getattr(Transcripts, field_name).desc())
                else:
                    if hasattr(Transcripts, sort):
                        stmt = stmt.order_by(getattr(Transcripts, sort))
            else:
                stmt = stmt.order_by(Transcripts.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Transcripts)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Transcripts]:
        """Update transcripts"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Transcripts {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated transcripts {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating transcripts {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Transcripts {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted transcripts {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting transcripts {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Transcripts]:
        """Get transcripts by any field"""
        try:
            if not hasattr(Transcripts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Transcripts")
            column = getattr(Transcripts, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Transcripts).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching transcripts by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Transcripts]:
        """Get list of transcriptss filtered by field"""
        try:
            if not hasattr(Transcripts, field_name):
                raise ValueError(f"Field {field_name} does not exist on Transcripts")
            column = getattr(Transcripts, field_name)
            ...
            logger.error(f"Error fetching transcriptss by {field_name}: {str(e)}")
            raise