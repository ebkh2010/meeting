...
# ------------------ Service Layer ------------------
class Meeting_archive_filesService:
    """Service layer for Meeting_archive_files operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Meeting_archive_files]:
        """Create a new meeting_archive_files"""
        try:
            obj = Meeting_archive_files(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created meeting_archive_files with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating meeting_archive_files: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Meeting_archive_files]:
        """Get meeting_archive_files by ID"""
        try:
            query = select(Meeting_archive_files).where(Meeting_archive_files.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meeting_archive_files {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of meeting_archive_filess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Meeting_archive_files, field):
                        column = getattr(Meeting_archive_files, field)
            ...
            stmt = select(Meeting_archive_files, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Meeting_archive_files, field_name):
                        stmt = stmt.order_by(getattr(Meeting_archive_files, field_name).desc())
                else:
                    if hasattr(Meeting_archive_files, sort):
                        stmt = stmt.order_by(getattr(Meeting_archive_files, sort))
            else:
                stmt = stmt.order_by(Meeting_archive_files.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Meeting_archive_files)
                for condition in conditions:
            ...
            logger.error(f"Error fetching meeting_archive_files list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Meeting_archive_files]:
        """Update meeting_archive_files"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meeting_archive_files {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated meeting_archive_files {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating meeting_archive_files {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete meeting_archive_files"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meeting_archive_files {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted meeting_archive_files {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting meeting_archive_files {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Meeting_archive_files]:
        """Get meeting_archive_files by any field"""
        try:
            if not hasattr(Meeting_archive_files, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meeting_archive_files")
            column = getattr(Meeting_archive_files, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Meeting_archive_files).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meeting_archive_files by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Meeting_archive_files]:
        """Get list of meeting_archive_filess filtered by field"""
        try:
            if not hasattr(Meeting_archive_files, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meeting_archive_files")
            column = getattr(Meeting_archive_files, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Meeting_archive_files.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching meeting_archive_filess by {field_name}: {str(e)}")
            raise