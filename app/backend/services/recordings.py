...
class RecordingsService:
    """Service layer for Recordings operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Recordings]:
        ...
        try:
            ...
            logger.info(f"Created recordings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating recordings: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Recordings]:
        """Get recordings by ID"""
        try:
            query = select(Recordings).where(Recordings.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching recordings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of recordingss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Recordings, field):
                        column = getattr(Recordings, field)
            ...
            stmt = select(Recordings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Recordings, field_name):
                        stmt = stmt.order_by(getattr(Recordings, field_name).desc())
                else:
                    if hasattr(Recordings, sort):
                        stmt = stmt.order_by(getattr(Recordings, sort))
            else:
                stmt = stmt.order_by(Recordings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Recordings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching recordings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Recordings]:
        """Update recordings"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Recordings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated recordings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating recordings {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Recordings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted recordings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting recordings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Recordings]:
        """Get recordings by any field"""
        try:
            if not hasattr(Recordings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Recordings")
            column = getattr(Recordings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Recordings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching recordings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Recordings]:
        """Get list of recordingss filtered by field"""
        try:
            if not hasattr(Recordings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Recordings")
            column = getattr(Recordings, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Recordings.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching recordingss by {field_name}: {str(e)}")
            raise