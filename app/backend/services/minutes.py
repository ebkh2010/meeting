...
class MinutesService:
    """Service layer for Minutes operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Minutes]:
        ...
        try:
            ...
            logger.info(f"Created minutes with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating minutes: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Minutes]:
        """Get minutes by ID"""
        try:
            query = select(Minutes).where(Minutes.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching minutes {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of minutess"""
        try:
            ...
            stmt = select(Minutes, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Minutes, field_name):
                        stmt = stmt.order_by(getattr(Minutes, field_name).desc())
                else:
                    if hasattr(Minutes, sort):
                        stmt = stmt.order_by(getattr(Minutes, sort))
            else:
                stmt = stmt.order_by(Minutes.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Minutes)
                for condition in conditions:
            ...
            logger.error(f"Error fetching minutes list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Minutes]:
        """Update minutes"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Minutes {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated minutes {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating minutes {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Minutes {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted minutes {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting minutes {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Minutes]:
        """Get minutes by any field"""
        try:
            if not hasattr(Minutes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Minutes")
            column = getattr(Minutes, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Minutes).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching minutes by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Minutes]:
        """Get list of minutess filtered by field"""
        try:
            if not hasattr(Minutes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Minutes")
            column = getattr(Minutes, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Minutes.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching minutess by {field_name}: {str(e)}")
            raise