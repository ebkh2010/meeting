...
async def load_app_user(db: AsyncSession, app_user_id: int) -> App_users:
    ...
    # سازمان منتقل‌شده به سطل آشغال پلتفرم از دسترس همهٔ اعضا خارج است؛ ۴۰۱ برمی‌گردد
    # تا نشست در سمت مرورگر پاک شود و کاربر به‌جای حلقهٔ ریدایرکت، به صفحهٔ ورود
    # منتقل شود.
    if await org_is_trashed(db, int(app_user.organization_id)):
        raise unauthorized(
            "سازمان شما توسط مدیریت پلتفرم به سطل آشغال منتقل شده است؛ نشست شما باطل شد. "
            "برای اطلاعات بیشتر با پشتیبانی ویدارا - نسخه جلسات تماس بگیرید."
        )
    return app_user
