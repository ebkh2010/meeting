...
# ------------------ Service Layer ------------------
class Org_upload_limitsService:
    """Service layer for Org_upload_limits operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Org_upload_limits]:
        """Create a new org_upload_limits"""
        try:
            obj = Org_upload_limits(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created org_upload_limits with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating org_upload_limits: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Org_upload_limits]:
        """Get org_upload_limits by ID"""
        try:
            query = select(Org_upload_limits).where(Org_upload_limits.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_upload_limits {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of org_upload_limitss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Org_upload_limits, field):
                        column = getattr(Org_upload_limits, field)
            ...
            stmt = select(Org_upload_limits, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Org_upload_limits, field_name):
                        stmt = stmt.order_by(getattr(Org_upload_limits, field_name).desc())
                else:
                    if hasattr(Org_upload_limits, sort):
                        stmt = stmt.order_by(getattr(Org_upload_limits, sort))
            else:
                stmt = stmt.order_by(Org_upload_limits.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Org_upload_limits)
                for condition in conditions:
            ...
            logger.error(f"Error fetching org_upload_limits list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Org_upload_limits]:
        """Update org_upload_limits"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_upload_limits {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated org_upload_limits {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating org_upload_limits {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete org_upload_limits"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Org_upload_limits {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted org_upload_limits {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting org_upload_limits {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Org_upload_limits]:
        """Get org_upload_limits by any field"""
        try:
            if not hasattr(Org_upload_limits, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_upload_limits")
            column = getattr(Org_upload_limits, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Org_upload_limits).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching org_upload_limits by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Org_upload_limits]:
        """Get list of org_upload_limitss filtered by field"""
        try:
            if not hasattr(Org_upload_limits, field_name):
                raise ValueError(f"Field {field_name} does not exist on Org_upload_limits")
            column = getattr(Org_upload_limits, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Org_upload_limits.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching org_upload_limitss by {field_name}: {str(e)}")
            raise