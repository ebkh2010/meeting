...
class OrganizationsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Organizations, field):
                        column = getattr(Organizations, field)
            ...
            stmt = select(Organizations, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Organizations, field_name):
                        stmt = stmt.order_by(getattr(Organizations, field_name).desc())
                else:
                    if hasattr(Organizations, sort):
                        stmt = stmt.order_by(getattr(Organizations, sort))
            else:
                stmt = stmt.order_by(Organizations.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Organizations)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Organizations]:
        ...
        try:
            ...
            column = getattr(Organizations, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Organizations).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
