...
# ------------------ Service Layer ------------------
class Meeting_attachmentsService:
    """Service layer for Meeting_attachments operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Meeting_attachments]:
        """Create a new meeting_attachments"""
        try:
            obj = Meeting_attachments(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created meeting_attachments with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating meeting_attachments: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Meeting_attachments]:
        """Get meeting_attachments by ID"""
        try:
            query = select(Meeting_attachments).where(Meeting_attachments.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meeting_attachments {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of meeting_attachmentss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Meeting_attachments, field):
                        column = getattr(Meeting_attachments, field)
            ...
            stmt = select(Meeting_attachments, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Meeting_attachments, field_name):
                        stmt = stmt.order_by(getattr(Meeting_attachments, field_name).desc())
                else:
                    if hasattr(Meeting_attachments, sort):
                        stmt = stmt.order_by(getattr(Meeting_attachments, sort))
            else:
                stmt = stmt.order_by(Meeting_attachments.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Meeting_attachments)
                for condition in conditions:
            ...
            logger.error(f"Error fetching meeting_attachments list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Meeting_attachments]:
        """Update meeting_attachments"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meeting_attachments {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated meeting_attachments {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating meeting_attachments {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete meeting_attachments"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Meeting_attachments {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted meeting_attachments {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting meeting_attachments {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Meeting_attachments]:
        """Get meeting_attachments by any field"""
        try:
            if not hasattr(Meeting_attachments, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meeting_attachments")
            column = getattr(Meeting_attachments, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Meeting_attachments).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching meeting_attachments by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Meeting_attachments]:
        """Get list of meeting_attachmentss filtered by field"""
        try:
            if not hasattr(Meeting_attachments, field_name):
                raise ValueError(f"Field {field_name} does not exist on Meeting_attachments")
            column = getattr(Meeting_attachments, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Meeting_attachments.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching meeting_attachmentss by {field_name}: {str(e)}")
            raise